...
<div class="container mx-auto px-4 py-12">
    ...
    <form method="post" action="{{ route('admin.posts.update',$post) }}" enctype="multipart/form-data" class="grid md:grid-cols-2 gap-4 mt-6">
        ...
        <div class="md:col-span-2">
            ...
            <input name="title" value="{{ $post->title }}" class="border p-3 rounded w-full" required>
        </div>
        ...
        <div>
            ...
            <select name="service_id" class="border p-3 rounded w-full" required>
                ...
                @foreach($services as $service)
                    <option value="{{ $service->id }}" {{ $post->service_id == $service->id ? 'selected' : '' }}>{{ $service->title }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <select name="tags[]" multiple class="border p-3 rounded w-full h-32">
                @foreach($tags as $tag)
                    <option value="{{ $tag->id }}" {{ $post->relatedTags->contains($tag->id) ? 'selected' : '' }}>{{ $tag->name }}</option>
                @endforeach
            </select>
        ...
        <div class="md:col-span-2">
            ...
            <input type="text" name="gallery_id" id="gallery_id" value="{{ $post->gallery_id }}" class="opacity-0 h-0 w-0 absolute" required>
            <div id="image_preview" class="{{ $post->image ? '' : 'hidden' }} mt-2">
                <img src="{{ $post->image }}" class="h-32 w-auto rounded border object-cover">
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="content" class="border p-3 rounded w-full md:col-span-2" rows="8" required>{{ $post->content }}</textarea>
        </div>

        <!-- Meta & Date -->
        <input name="published_at" type="date" value="{{ optional($post->published_at)->format('Y-m-d') }}" class="border p-3 rounded">
        <input name="meta_title" value="{{ $post->meta_title }}" class="border p-3 rounded" placeholder="عنوان ميتا">
        <input name="meta_description" value="{{ $post->meta_description }}" class="border p-3 rounded" placeholder="وصف ميتا">

        <button class="px-5 py-3 bg-primary text-white rounded md:col-span-2 hover:bg-primary-dark">حفظ التعديلات</button>
    </form>
</div>
