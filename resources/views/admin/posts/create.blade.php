...
<div class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">قم بملء البيانات التالية لإضافة مقال جديد للموقع</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" value="{{ old('title') }}" class="w-full px-4 py-3 rounded-lg border-gray-300 border focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors shadow-sm @error('title') border-red-500 @enderror" placeholder="اكتب عنوان المقال هنا..." required>
                            @error('title')
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" class="w-full rounded-lg border-gray-300 border shadow-sm @error('content') border-red-500 @enderror" rows="15">{{ old('content') }}</textarea>
                            @error('content')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="meta_title" value="{{ old('meta_title') }}" class="w-full px-4 py-3 rounded-lg border-gray-300 border focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors shadow-sm @error('meta_title') border-red-500 @enderror" placeholder="يترك فارغاً لاستخدام عنوان المقال">
                            @error('meta_title')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="published_at" class="w-full px-4 py-2 rounded-lg border-gray-300 border focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors shadow-sm">
                                    ...
                                    <div class="space-y-2">
                                        @foreach($tags as $tag)
                                            <label class="flex items-center p-2 rounded hover:bg-gray-50 cursor-pointer transition-colors select-none">
                                                <input type="checkbox" name="tags[]" value="{{ $tag->id }}" class="w-4 h-4 text-green-600 rounded border-gray-300 focus:ring-green-500 ml-3">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6">
                        <input type="text" name="gallery_id" id="gallery_id" value="{{ old('gallery_id') }}" class="opacity-0 h-0 w-0 absolute">
                        
                        <div id="image_preview_container" class="space-y-4">
                            <!-- Placeholder -->
                            <div id="image_placeholder" class="border-2 border-dashed {{ $errors->has('gallery_id') ? 'border-red-300 bg-red-50' : 'border-gray-300' }} rounded-xl p-8 text-center hover:border-green-500 hover:bg-green-50 transition-all cursor-pointer group" onclick="openGalleryPicker()">
                            ...
                            <div id="image_preview" class="hidden relative group rounded-xl overflow-hidden border border-gray-200 shadow-sm">
...
function updatePagination(data) {
    ...
    if (data.prev_page_url) {
        ...
        prevBtn.innerHTML = `
            ...
            السابق
        `;
        prevBtn.className = 'flex items-center px-3 py-1.5 bg-white border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50 transition-colors shadow-sm';
        prevBtn.onclick = () => {
            currentPage--;
            loadGalleryImages(currentPage);
        };
    ...
    if (data.next_page_url) {
        ...
        `;
        nextBtn.className = 'flex items-center px-3 py-1.5 bg-white border border-gray-300 rounded-md text-sm text-gray-700 hover:bg-gray-50 transition-colors shadow-sm';
        nextBtn.onclick = () => {
            currentPage++;
            loadGalleryImages(currentPage);
        };
