...
<div class="container mx-auto px-4 py-28">
    ...
    <form method="post" action="{{ route('admin.services.update',$service) }}" enctype="multipart/form-data" class="grid md:grid-cols-2 gap-4 mt-6">
        ...
        <div class="border p-3 rounded">
            ...
            @if($service->image)
                <div class="mb-2">
                    <img src="{{ $service->image }}" class="h-20 w-auto rounded border">
                </div>
            @endif
        ...
        <div class="border p-3 rounded md:col-span-2">
            ...
            <div id="service_gallery_preview" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
                @if($service->galleries)
                    @foreach($service->galleries as $gallery)
                        <div class="relative group border rounded h-24 w-full overflow-hidden">
                            <img src="{{ $gallery->image_url }}" class="w-full h-full object-cover">
                            <button type="button" onclick="this.parentElement.remove(); removeGalleryId({{ $gallery->id }})" class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity">
...
<script>
    ...
    function closeGalleryPicker() {
        ...
        if (pickerMode === 'multiple') {
            selectedGalleryItems = []; // Reset if closed without confirming
        }
    }
