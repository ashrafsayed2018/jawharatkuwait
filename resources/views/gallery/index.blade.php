        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" id="gallery-grid">
            @forelse($images as $image)
                <div class="relative group overflow-hidden rounded-lg shadow-lg cursor-pointer transition-transform duration-300 hover:scale-[1.02]" onclick="openLightbox('{{ $image->image_url }}', '{{ $image->title }}')">
                    ...
                    @auth
                        <button type="button" onclick="copyImageUrl(event, this, '{{ $image->image_url }}')"
                                title="نسخ رابط الصورة"
                                class="copy-btn absolute top-2 left-2 z-10 bg-white/90 hover:bg-white text-gray-700 hover:text-primary rounded-lg p-2 shadow-md transition-colors duration-300">
                            ...
                            <svg class="check-icon h-5 w-5 hidden text-green-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<script>
    ...
    function copyImageUrl(event, button, url) {
        ...
        const showButtonFeedback = () => {
            ...
            clearTimeout(button.__resetTimeout);
            button.__resetTimeout = setTimeout(() => {
        ...
        const onSuccess = () => {
            showButtonFeedback();
            showToast('تم نسخ رابط الصورة', false);
        };

        const onFailure = () => {
            showToast('تعذر نسخ الرابط، حاول مرة أخرى', true);
        };
