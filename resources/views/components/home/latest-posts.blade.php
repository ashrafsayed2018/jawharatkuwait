...
<section class="container mx-auto px-4 py-12">
    ...
    <div class="grid md:grid-cols-3 gap-6 mt-6">
        @forelse($posts as $post)
            <x-post.card :href="route('blog.show',$post->slug)" :title="$post->title" :image="$post->image" :excerpt="$post->meta_description ?? Str::limit(strip_tags($post->content), 120)" />
        @empty
            @php($fallback = [
                ...
                ['title' => 'تجهيز المناسبات والبوفيهات', 'desc' => 'تنظيم وتقديم بوفيهات متنوعة مع لمسات فنية مميزة.', 'img' => 'images/جوهرة الكويت لتأجير طاولات وكراسي/جوهرة الكويت للضيافة/جوهرة الكويت لتأجير طاولات وكراسي (1).webp'],
