...
<header class="border-b">
    <div class="container mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-4">
            ...
            <a href="{{ route('gallery.index') }}" class="rounded px-2 py-1 {{ request()->routeIs('gallery.*') ? 'bg-green-600 text-white' : 'hover:text-primary' }}">معرض الصور</a>
        ...
        <div class="hidden md:flex items-center gap-2">
            ...
            <a href="https://wa.me/{{ Str::startsWith($whatsapp, '965') ? $whatsapp : '965'.$whatsapp }}" target="_blank" class="px-3 py-2 rounded bg-green-600 text-white">واتساب</a>
        </div>
    </div>
</header>
...
<div id="navPanel" class="hidden fixed top-0 right-0 w-64 h-full bg-white z-50 shadow-lg transform translate-x-full transition">
    ...
    <nav class="flex flex-col p-4 gap-3">
        ...
        <a href="{{ route('gallery.index') }}" class="px-3 py-2 rounded hover:bg-gray-100 {{ request()->routeIs('gallery.*') ? 'bg-green-600 text-white' : '' }}">معرض الصور</a>
