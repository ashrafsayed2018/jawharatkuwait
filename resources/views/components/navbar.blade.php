                ...
                <a href="{{ route('home') }}" 
                   class="relative px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 group
                    ...
                    @if(request()->routeIs('home'))
                        <span class="absolute inset-0 bg-linear-to-r from-primary to-green-500 rounded-lg"></span>
                    @else
            ...
            <div class="hidden lg:flex items-center gap-3">
                <a href="tel:{{ $phone }}" 
                   class="group relative px-5 py-2.5 rounded-xl font-medium text-sm overflow-hidden transition-all duration-300 hover:shadow-lg hover:scale-105">
                    <span class="absolute inset-0 bg-linear-to-r from-primary to-green-500"></span>
                ...
                <a href="https://wa.me/{{ Str::startsWith($whatsapp, '965') ? $whatsapp : '965'.$whatsapp }}" 
                   target="_blank"
                   class="group relative px-5 py-2.5 rounded-xl font-medium text-sm overflow-hidden transition-all duration-300 hover:shadow-lg hover:scale-105">
                    <span class="absolute inset-0 bg-linear-to-r from-green-500 to-green-600"></span>
                    <span class="relative z-10 text-white flex items-center gap-2">
...
<div id="navPanel" class="hidden fixed top-0 right-0 w-80 h-full bg-white z-50 shadow-2xl transform translate-x-full transition-transform duration-300">
    <!-- Panel Header -->
    <div class="p-5 flex items-center justify-between border-b border-gray-100 bg-linear-to-r from-primary/5 to-green-50">
        <a href="/">
    ...
    <nav class="flex flex-col p-4 gap-2 overflow-y-auto" style="max-height: calc(100vh - 180px);">
        <a href="{{ route('home') }}" 
           class="px-4 py-3 rounded-xl font-medium transition-all duration-300 {{ request()->routeIs('home') ? 'bg-linear-to-r from-primary to-green-500 text-white shadow-lg' : 'hover:bg-gray-50 text-gray-700' }}">
            الرئيسية
        </a>
        <a href="{{ route('services.index') }}" 
           class="px-4 py-3 rounded-xl font-medium transition-all duration-300 {{ request()->routeIs('services.*') ? 'bg-linear-to-r from-primary to-green-500 text-white shadow-lg' : 'hover:bg-gray-50 text-gray-700' }}">
            الخدمات
        </a>
        <a href="{{ route('about') }}" 
           class="px-4 py-3 rounded-xl font-medium transition-all duration-300 {{ request()->routeIs('about') ? 'bg-linear-to-r from-primary to-green-500 text-white shadow-lg' : 'hover:bg-gray-50 text-gray-700' }}">
            من نحن
        </a>
        <a href="{{ route('blog.index') }}" 
           class="px-4 py-3 rounded-xl font-medium transition-all duration-300 {{ request()->routeIs('blog.*') ? 'bg-linear-to-r from-primary to-green-500 text-white shadow-lg' : 'hover:bg-gray-50 text-gray-700' }}">
            المدونة
        </a>
        <a href="{{ route('gallery.index') }}" 
           class="px-4 py-3 rounded-xl font-medium transition-all duration-300 {{ request()->routeIs('gallery.*') ? 'bg-linear-to-r from-primary to-green-500 text-white shadow-lg' : 'hover:bg-gray-50 text-gray-700' }}">
            معرض الصور
        </a>
        <a href="{{ route('contact.index') }}" 
           class="px-4 py-3 rounded-xl font-medium transition-all duration-300 {{ request()->routeIs('contact.*') ? 'bg-linear-to-r from-primary to-green-500 text-white shadow-lg' : 'hover:bg-gray-50 text-gray-700' }}">
            تواصل
        </a>
    </nav>

    <!-- Panel CTA Buttons -->
    <div class="absolute bottom-0 left-0 right-0 p-4 bg-linear-to-t from-white via-white to-transparent">
        <div class="flex flex-col gap-3">
            <a href="tel:{{ $phone }}" 
               class="px-4 py-3 rounded-xl font-medium text-center bg-linear-to-r from-primary to-green-500 text-white shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2">
            ...
            <a href="https://wa.me/{{ $whatsapp }}" 
               target="_blank"
               class="px-4 py-3 rounded-xl font-medium text-center bg-linear-to-r from-green-500 to-green-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center gap-2">
